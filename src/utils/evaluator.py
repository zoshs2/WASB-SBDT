...
class Evaluator(object):
    ...
    def accuracy(self):
        accuracy = 0.0
        denom = self.tp_all + self.tn_all + self.fp_all + self.fn_all
        if denom > 0.0:
            accuracy = (self.tp_all + self.tn_all) / denom
        return accuracy
    ...
    def ap(self):
        """
        Average Precision.
        GT 양성 (tp_all + fn_all)이 하나도 없거나,
        스코어가 비어 있으면 AP는 정의되지 않으므로 0.0으로 처리.
        """
        total_pos = self.tp_all + self.fn_all
        if total_pos == 0 or len(self._scores) == 0:
            return 0.0

        scores = np.array(self._scores, dtype=float)
        inds = np.argsort(-scores).tolist()
